...
- **Two real bugs were found and fixed during implementation**, via live verification against the actual running backend (not just mocked tests), and both were caught by the subagent-driven-development review loop — task-scoped reviewers using live/real verification, not just diff-reading — before merge:
